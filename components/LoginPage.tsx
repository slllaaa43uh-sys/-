...
import { Lock, Mail, ArrowLeft, X, User, Building2, ChevronRight, Check, ArrowRight as ArrowRightIcon, Briefcase, Globe, Loader2, Send, ShieldCheck, MapPin, Phone, Paste } from 'lucide-react';
...
const LoginPage: React.FC<LoginPageProps> = ({ onLoginSuccess }) => {
  ...
  useEffect(() => {
    if (registerStep === 'form' && !regCountry) {
        ...
        fetch(`${API_BASE_URL}/api/v1/auth/detect-country`)
            ...
            .catch(() => {
                // Ignore error, let user select manually
            })
            .finally(() => {
  ...
  // Function to actually submit the verification code
  const submitVerificationCode = async (code: string) => {
    ...
    try {
        ...
        if (response.ok) {
            ...
            setIsVerifying(false); // Only stop loading on error, let user retry
        }
    } catch (e) {
  ...
  const handleLongPressStart = (index: number) => {
    // Only trigger for the "last" visual box (index 5 because we force LTR for numbers)
    if (index === 5) {
  ...
  const handleRegisterSubmit = async () => {
    ...
    if (regCountry === 'السعودية') {
        ...
        // Auto-prepend code if selected and user didn't type it
        if (phoneCode && !regPhone.startsWith('+')) {
             // Remove leading zeros if present before appending code
                                ...
                                <div className="relative">
                                    ...
                                    >
                                        ...
                                        {ARAB_LOCATIONS.map((loc) => (
                                            // Value uses Arabic (Backend), Label uses Language (Display)
                    ...
                    <div className="space-y-6 text-center">
                        ...
                        <div className="flex justify-center gap-2 dir-ltr">
                            {Array.from({ length: 6 }).map((_, index) => (
                                <input
                                    ...
                                    onContextMenu={(e) => {
                                        // Prevent default context menu on the last box to allow our custom long press logic
                                        if (index === 5) e.preventDefault();
                                    }}
                                    // UPDATED CLASS: h-12 w-12, text-xl (Reduced size & caret height)
                        ...
                        <div className="space-y-3">
                            <button
                                ...
                                className="w-full py-3.5 bg-gradient-to-r from-blue-600 to-green-500 hover:from-blue-700 hover:to-green-600 text-white rounded-xl font-bold text-base shadow-lg shadow-green-500/20 active:scale-[0.98] transition-all flex items-center justify-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                            >
